...
class MasterGrader:
    ...
    def step1_extraction(self) -> bool:
        ...
        try:
            ...
            print(f"[+] تعداد دانشجویان پردازش شده: {len(self.extraction_results)}")
    ...
    def step2_organization(self) -> bool:
        ...
        try:
            ...
            print(f"[+] تعداد دانشجویان سازماندهی شده: {len(self.organization_results)}")
    ...
    def step4_reporting(self) -> bool:         """         مرحله 4: تولید گزارش‌ها
                Returns:             True اگر موفق بود       د """       " try:            report _gen = reporter.Reporter()            report _gen.generate_all_reports(self.plagiarism_cases, self.statistics)            
            # نوشتن فایل لاگ             reporter.write_log_file(self.log_entries)            
            return True
                    except Exception as e:           c print(f"\n[-] خطا در مرحله گزارش‌دهی: {str(e)}")           ه return F_codealnews</e
(e)}")
            return False
    ...
    def cleanup_temp_dirs(self):         """پاکسازی پوشه‌های موقت"""         print(f"\n[INFO] در حال پاکسازی {len(self.temp_dirs)} پوشه موقت...")         for temp_dir in self.temp_dirs:             if temp_dir and os.path.exists(temp_dir):                 try:                     shutilutil.rmtree(temp_dir, ignore_errors=True)
                except:
                    pass
    ...
    def print_final_summary(self):
        ...
        print("="*80)
        print(f"\n[SUMMARY] خلاصه:")
        print(f"  • دانشجویان پردازش شده: {len(self.extraction_results)}")
        print(f"  • فایل‌های سازماندهی شده: {sum(r.get('total_files', 0) for r in self.organization_results.values())}")
        print(f"  • موارد تقلب شناسایی شده: {len(self.plagiarism_cases)}")
        print(f"  • خطاها: {len(self.log_entries)}")
        print(f"\n[OUTPUT] فایل‌های خروجی:")
        print(f"  • پوشه فایل‌های سازماندهی شده: {config.Config.OUTPUT_DIR}")
        print(f"  • گزارش CSV: {config.Config.get_report_file_path()}")
        print(f"  • گزارش تفصیلی: {os.path.join(config.Config.OUTPUT_DIR, 'Detailed_Report.txt')}")
        print(f"  • فایل لاگ: {config.Config.get_log_file_path()}")
        print("\n" + "="*80)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='MasterGrader - سیستم جامع تصحیح و تشخیص تقلب تمرینات C',
...
مثال‌ها:
  ...
  python main.py --input "C:/submissions" --threshold 90 --template "template.c"
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help=f'آستانه شباهت برای تشخیص تقلب (درصد، پیش‌فرض: {config.Config.SIMILARITY_THRESHOLD})'
    )
