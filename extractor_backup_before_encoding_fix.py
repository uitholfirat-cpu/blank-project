    ...
    def extract_student_zips_to_temp(self, student_dir: str, student_id: str) -> Optional[str]:
        ...
        try:
            ...
            if not zip_files_found:
                ...
                if c_files_copied > 0:
                    print(f"    [INFO] {c_files_copied} فایل C کپی شد (بدون ZIP)")
            ...
            if not files_in_temp:                 print(f"    [WARN] هشدار: پوشه موقت خالی است برای {student_id}")           e else:                 c_files_count = sum(1 for f in files_in_temp if os.path.splitext(f)[1].lower() in config.Config.ACCEPTED_EXTENSIONS)                 if c_files_count == 0:                     print(f"    [WARN] هشدار: هیچ فایل C در پوشه موقت پیدا نشد (تعداد کل فایل‌ها: {len(files_in_temp)_code})new"</)
emp)})")
...
def extract_student_submissions(root_dir: str, log_callback=None) -> Dict[str, Dict]:
    ...
    for item in os.listdir(root_dir):
        ...
        if os.path.isdir(student_path):             student_id = item             print(f"[INFO] در حال استخراج فایل‌های {student_id}...")            
            # استخراج به پوشه موقت             temp_dir = extractor.extract_student_zips_to_temp(student_path, student_id)            
            if temp_dir:                 results[student_id] = {                     'temp_path': temp_dir,  # مسیر پوشه موقت                     'original_path': student_path,  # مسیر اصلی                     'failed': extractor.get_failed_extractions()                 }                 print(f"  [+] فایل‌ها در پوشه موقت استخراج شدند")            else :                 results[student_id] = {                     'temp_path': None,                     'original_path': student_path,                     'failed': extractor.get_failed_extractions()                }
                print(f"  [WARN] خطا در استخراجnew"</)
f"  ⚠ خطا در استخراج")
